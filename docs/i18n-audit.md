...
| `Save Local Draft`, `Open in Viewer (same tab)`, `Import package (.zip) / legacy JSON`, `Export package (.zip)` | `server/editor/main.js` | Editor main actions | translate-now | Core fixed buttons | `editor.actions.saveLocalDraft`, `editor.actions.openViewer`, `editor.actions.importPackage`, `editor.actions.exportPackage` |
