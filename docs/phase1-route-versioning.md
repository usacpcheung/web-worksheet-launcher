...
Access mode for those later routes should follow capability needs rather than route name alone: purely local/read-only experiences can remain public, while any route that uses rewrite, T2A, autosave, publish, versioned storage, or persisted learner data must add the appropriate authentication and backend authorization.
