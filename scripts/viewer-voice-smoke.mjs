...
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').evaluate(e=>e.readOnly),true);
...
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').evaluate(e=>e.readOnly),false);
await page.locator('.question-card > textarea:not(.question-card__review-status)').fill('Question two typing');
...
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').inputValue(),'Question two typing');
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').evaluate(e=>e===document.activeElement),true);
...
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').inputValue(),'Earlier answer');
await page.locator('.question-card > textarea:not(.question-card__review-status)').click();
await page.locator('.question-card > textarea:not(.question-card__review-status)').evaluate(e=>{e.setSelectionRange(0,7);e.dispatchEvent(new Event('select'));});
await page.getByRole('button',{name:'Add by voice',exact:true}).click();
await page.getByRole('button',{name:'Stop',exact:true}).click();
await page.waitForFunction(()=>!window.viewerSession.voice.active);
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').inputValue(),'Earlier New segment answer');
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').evaluate(e=>e.readOnly),false);
...
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').inputValue(),'Earlier New segment answer New segment');
await page.locator('.question-card > textarea:not(.question-card__review-status)').fill('Edited after voice');
await page.getByRole('button',{name:'Go to next block',exact:true}).click();
await page.getByRole('button',{name:'Go to previous block',exact:true}).click();
assert.equal(await page.locator('.question-card > textarea:not(.question-card__review-status)').inputValue(),'Edited after voice');
failRewrite=true;
await page.getByRole('button',{name:'Add by voice',exact:true}).click();
await page.getByRole('button',{name:'Stop',exact:true}).click();
await page.waitForFunction(()=>!window.viewerSession.voice.active);
...
await page.locator('.question-card > textarea:not(.question-card__review-status)').fill('Current answer after signing in');
